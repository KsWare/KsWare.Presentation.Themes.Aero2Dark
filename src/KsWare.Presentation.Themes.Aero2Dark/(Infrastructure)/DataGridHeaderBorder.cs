...
// Microsoft.Windows.Themes.DataGridHeaderBorder; PresentationFrameworkAero2, 8.0.0.0
	...
	public partial class DataGridHeaderBorder {
		...
		private void RenderTheme(DrawingContext dc) {
			...
			if (size.Width >= 2.0) {
				...
				if (isPressed) sideType = AeroFreezables.PressedSides;
				else if (isHovered) sideType = AeroFreezables.HoveredSides;
				else if (isSorted || isSelected) sideType = AeroFreezables.SortedSides;
				

				if (SeparatorVisibility == Visibility.Visible) {
					Brush sideBrush;
					if (SeparatorBrush != null) sideBrush = SeparatorBrush;
					else {
			...
			if (isPressed && (size.Width >= 4.0) && (size.Height >= 4.0)) {
				...
				var topBrush =
					(LinearGradientBrush) GetCachedFreezable((int) AeroFreezables.PressedTop);
				if (topBrush == null) {
				...
				var pressedBevel =
					(LinearGradientBrush) GetCachedFreezable((int) AeroFreezables.PressedBevel);
			...
			if (isSorted && (size.Width > 14.0) && (size.Height > 10.0)) {
				...
				var arrowGeometry = (PathGeometry) GetCachedFreezable(ascending
					? (int) AeroFreezables.ArrowUpGeometry
					: (int) AeroFreezables.ArrowDownGeometry);
				...
				var arrowFill =
					(LinearGradientBrush) GetCachedFreezable((int) AeroFreezables.ArrowFill);
				if (arrowFill == null) {
...
namespace KsWare.Presentation.Themes.Aero2Dark {
	...
	public sealed partial class DataGridHeaderBorder : Border {
		...
		static DataGridHeaderBorder() {
			...
			SnapsToDevicePixelsProperty.OverrideMetadata(typeof(DataGridHeaderBorder),
		...
		private bool UsingBorderImplementation {
			// WORKAROUND replaced: get { return (Background != null) || (BorderBrush != null); }
			get { return false; }
		}
		...
		protected override Size MeasureOverride(Size constraint) {
			...
			if (child != null) {
				...
				return new Size(desiredSize.Width + padding.Left + padding.Right,
